...
const MovieInfo = (props: MovieInfoProps) => {
 ...
 return (
  <>
   {props.release && <p>({new Date(props.release).getFullYear()})</p>}
   {posterPhoto.imageUrl && (
    <img
     src={getUrl(posterPhoto.imageUrl, props.meta)}
...
const Cast = (props: CastProps) => (
 <div>
  ...
  <ul style={{ listStyle: 'none', display: 'flex', flexFlow: 'row wrap' }}>
   {props.cast.map(
    (person: CastMemberProps, i: number) =>
     person && <CastMember key={i} {...person} meta={props.meta} />
   )}
  </ul>
 </div>
);
 ...
 return (
  <li style={{ marginRight: '15px' }}>
   {personPhoto.imageUrl && (
    <img
     src={getUrl(personPhoto.imageUrl, meta)}
